...
chapters/ch{NN}-{chapter_slug}/
  tutorial.docx           # primary chapter document
  tutorial.handoff.json   # see §5
  diagrams/
...
(`tutorial.docx`, `tutorial.handoff.json`) inside `chapters/ch{NN}-{chapter_slug}/`, with
no course-slug or chapter-slug prefix. This keeps paths within the Windows
260-char limit.

## 5. Handoff Manifest (`tutorial.handoff.json`)
...
- [ ] `tutorial.handoff.json` exists and is internally consistent with the
      doc body.
