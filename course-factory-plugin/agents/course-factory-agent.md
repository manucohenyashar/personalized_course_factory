...
description: Master entry point for the Personalized Course Factory. Accepts course specifications inline (domain, students, subject) or uses pre-filled inputs/ files, then orchestrates the complete pipeline end-to-end across planning, environment setup, all chapters, evaluation, the capstone lab, chapter podcasts (NotebookLM), and the README. Invoke this agent to generate a complete course from scratch or to resume a previously started run.
