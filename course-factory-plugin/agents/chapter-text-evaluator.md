...
You receive:
- `chapter_doc_path`: path to `tutorial.docx` (inside `chapters/ch{NN}-{slug}/`)
- `handoff_json_path`: path to `tutorial.handoff.json` (inside `chapters/ch{NN}-{slug}/`)
