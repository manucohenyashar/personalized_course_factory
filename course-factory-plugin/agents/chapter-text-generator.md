...
Execute the full Personalization Protocol (Steps P1–P4 in ${CLAUDE_PLUGIN_ROOT}/course-factory-guide.md) before writing any section.
...
You receive the full **common input envelope** (see ${CLAUDE_PLUGIN_ROOT}/course-factory-guide.md), plus:
...
Apply Word formatting conventions per `${CLAUDE_PLUGIN_ROOT}/doc/DocxDesignSpec.md`:
...
using the schema in ${CLAUDE_PLUGIN_ROOT}/course-factory-guide.md (Handoff JSON Schema section). Populate all fields.
