...
`${CLAUDE_PLUGIN_ROOT}/doc/GreatQuizSpec.md`. Run the skill `/generate-quiz` for item-by-item generation rules,
distractor patterns, and the difficulty heuristic formula.

## Personalization

Execute the full Personalization Protocol (Steps P1–P4 in ${CLAUDE_PLUGIN_ROOT}/course-factory-guide.md) before writing any item.
...
These are the files students actually see. They MUST follow `${CLAUDE_PLUGIN_ROOT}/doc/DocxDesignSpec.md`.
