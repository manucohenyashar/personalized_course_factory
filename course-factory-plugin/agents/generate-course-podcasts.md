...
description: Orchestrates creation of one NotebookLM podcast per chapter across an entire course. Uses a single shared NotebookLM notebook for the whole course, discovers chapter folders under a course root, and drives the batch podcast tool (${CLAUDE_PLUGIN_ROOT}/tools/notebooklm_podcast_gen.py) which uploads each chapter's tutorial.docx + slides.pdf + podcast-script.md, generates a scoped Audio Overview, and renames it. Continues past individual chapter failures and returns a summary report. Invoke after a course has been generated to batch-produce chapter podcasts.
...
**`${CLAUDE_PLUGIN_ROOT}/tools/notebooklm_podcast_gen.py`** (relative to the repo root). Your job is to validate
...
## Assumed folder structure (${CLAUDE_PLUGIN_ROOT}/course-factory-guide.md §5.2)
...
```
python ${CLAUDE_PLUGIN_ROOT}/tools/notebooklm_podcast_gen.py \
