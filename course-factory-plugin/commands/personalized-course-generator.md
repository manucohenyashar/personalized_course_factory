...
The agent drives `${CLAUDE_PLUGIN_ROOT}/tools/notebooklm_podcast_gen.py`, which creates the shared notebook, uploads
...
general generation agent) to write `README.docx` via the docx tooling, following `${CLAUDE_PLUGIN_ROOT}/doc/DocxDesignSpec.md`.
...
- Obey ${CLAUDE_PLUGIN_ROOT}/course-factory-guide.md Rule 1 & Rule 2: NO Bloom labels, LO-IDs, section numbers, `§`, chapter slugs,
