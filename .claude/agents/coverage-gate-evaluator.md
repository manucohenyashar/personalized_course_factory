...
- `handoff_json`: the chapter's `tutorial.handoff.json` (contains `learning_outcome_refs` and `section_outline`)
...
2. **Bloom tier coverage** — across the artifact:
   - For a **quiz**: must have items at Remember, Understand, Apply, Analyze, and at least one at Evaluate/Create (per the quiz Bloom distribution).
